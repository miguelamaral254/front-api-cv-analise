...
import FinalizandoVagaModal from './FinalizandoVagaModal';
...
const VagaDetalhes = ({vaga, onVoltarClick, onListarClick, onVagaFinalizada}) => {
        ...
        const handleFinalizarVaga = async () => {
            ...
            if (result.isConfirmed) {
                if (!user || !user.id) {
                    fireError('Erro de Autenticação!', 'Usuário não identificado. Por favor, faça login novamente.');
                    return;
                }

                setIsFinalizing(true);
                try {
                    await finalizeVaga(vaga.id, user.id);
        ...
        return (
            <>
                {isFinalizing && <FinalizandoVagaModal/>}
                        ...
                        <div className="flex flex-col items-start gap-3 w-full sm:w-auto">
                            ...
                            {isRecruiter && !vaga.finalizada_em && (
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 bg-red-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-700 transition-colors whitespace-nowrap text-sm w-full disabled:bg-gray-400"
                                >
                                    {isFinalizing ? 'Finalizando...' : <><MdLock/> Finalizar</>}
                                </button>
                            )}
                        </div>
                    </div>
                ...
                🏷️ {vaga.nome_area}
                            ...
                            {vaga.vaga_pcd && (
                                <p className="flex items-center">
                                    ...
                                    <span
                                        className="bg-purple-100 text-purple-800 px-2.5 py-1 rounded-full text-sm font-medium">♿ Vaga Afirmativa (PCD)</span>
                                </p>
                            )}
                            <p><strong className="font-semibold">Aberta
                                desde:</strong> {new Date(vaga.criado_em).toLocaleDateString()}</p>
                            {vaga.finalizada_em && (
                                <p><strong className="font-semibold">Finalizada
                                    em:</strong> {new Date(vaga.finalizada_em).toLocaleDateString()}</p>
                            )}
                        </div>
                    </div>
